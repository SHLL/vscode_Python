...
class JobsSpider(scrapy.spiders.Spider):
    ...
    def parse(self, response):
        list_urls = response.selector.xpath(
            "//*[contains(@class,'content pl-3 pr-3)]//*[contains(@class,'topic-title-wrap rm-tdu')]/@href"
        ).extract()
    ...
    def content(self, response):
        ...
        item['title'] = response.selector.xpath(
            '/html/body/div[2]/div[1]/div/div[1]/div[2]/div/div[3]/div[26]/div[1]/text()'
        ).extract_first()
        item['created_at'] = response.selector.xpath(
            '/html/body/div[2]/div[1]/div/div/div[1]/div[1]/div/p/a[2]/@data-tooltip'
        ).extract_first()
        yield item